...
contract('TestERC721Mintable', (accounts) => {
  ...
  describe('match erc721 spec', function () {
    beforeEach(async function () {
      this.contract = await ERC721Mintable.new(name, symbol, {
        from: account_one,
      });
    ...
    it('should get token balance', async function () {
      ...
      assert.equal(
        result_account_one,
        5,
        "Return balance of account_one isn't correct"
      );
      let result_account_two = await this.contract.balanceOf(account_two);
      assert.equal(
        result_account_two,
        0,
        "Return balance of account_two isn't correct"
      );
    });
    ...
    it('should transfer token from one owner to another', async function () {
      let tokenId = '1';
      let transferTx = await this.contract.transferFrom(
        account_one,
        account_two,
        tokenId
      );
  ...
  describe('have ownership properties', function () {
    ...
    it('should return contract owner', async function () {
      ...
      assert.equal(
        result,
        account_one,
        "Can't return contract owner correctly"
      );
    });
  });
});
